...
- ✅ Pluggable LLM/vectorizer providers (Ollama, OpenAI, Cohere)
...
- **In‑memory state** — the active LLM config and chat history reset on restart (the config can be re‑seeded via `OLLAMA_CHAT_MODEL`).
...
### Retrieval quality
- [ ] **De‑prioritize test files** in seed retrieval (down‑weight `*.test.*` / `*.spec.*` / `/tests/`) so implementation surfaces first. _(highest impact / lowest effort — next up)_
- [ ] **Per‑entity natural‑language descriptions** — generate and embed a short summary of each function/class so implementation code becomes retrievable for conceptual queries.
